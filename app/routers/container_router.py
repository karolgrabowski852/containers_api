...
from app.db.collections import users, containers
...
async def get_container(name: str, user: UserPublic = Depends(get_current_user)):
    container = await containers.find_one({"name": name, "owner_email": user.email})
    if not container:
...
async def create_container(
    ...
    existing = await containers.find_one({"name": name, "owner_email": user.email})
    if existing:
        raise HTTPException(status_code=409, detail="Container name already exists for this user")

    settings = resources.create_container(user.email, cpu, memory, gpu)
    if not settings:
    ...
    await containers.insert_one(container.model_dump())
...
async def delete_container(name: str, user: UserPublic = Depends(get_current_user)):
    ...
    resources.delete_container(ContainerSettings(**container["settings"]))
    await containers.delete_one({"name": name, "owner_email": user.email})
